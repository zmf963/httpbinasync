        ...
        for item in v:
            ...
            if files.get(k):
                if not isinstance(files[k], list):
                    files[k] = [files[k]]
                files[k].append({"name":item.name,"body":json_safe(item.body),"type":item.type})
            else:
                files[k] = {"name":item.name,"body":json_safe(item.body),"type":item.type}
    return files
...
def semiflatten(multi):
    """将MutiDict转换为常规dict。如果一个键有多个值，则结果将显示该键的值列表。否则它将具有普通值。"""
    if multi:
...
def get_url(request):
    ...
    protocol = request.headers.get(
        'X-Forwarded-Proto') or request.headers.get('X-Forwarded-Protocol')
...
def get_dict(request, *keys, **extras):
    ...
    _keys = ('url', 'args', 'form', 'data', 'origin',
    ...
    d = dict(
        ...
        form=request.form,
        data=json_safe(data),
        origin=request.headers.get('X-Forwarded-For', request.ip),
